...
// Timeline components - if @mui/lab is not available, we'll use a custom implementation
import {
  Edit as EditIcon,
  Save as SaveIcon,
...
import { invalidateDashboardCache, invalidateTicketQueries, invalidateWorkOrderQueries } from '../utils/cacheUtils.js'
...
export default function RepairTicketDetail() {
  ...
  const ticket = useQuery({
    ...
    queryFn: async () => (await api.get(`/repairTickets/${id}`)).data.data,
    enabled: !!id,
  })
  ...
  const updateTicket = useMutation({
    mutationFn: (updates) => api.patch(`/repairTickets/${id}`, updates),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['repairTicket', id] })
      queryClient.invalidateQueries({ queryKey: ['repairTickets'] })
      toast.success(translate('notifications.repairTicketUpdated'))
      setIsEditing(false)
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || translate('errors.failedToUpdateRepairTicket'))
    },
  })

  const deleteTicket = useMutation({
    mutationFn: () => api.delete(`/repairTickets/${id}`),
    onSuccess: async () => {
      ...
      toast.success(translate('notifications.repairTicketDeleted'))
      navigate('/non-warranty')
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || translate('errors.failedToDeleteRepairTicket'))
    },
  })

  const convertToWorkOrder = useMutation({
    mutationFn: (data) => api.post(`/repairTickets/${id}/convert`, data),
    onSuccess: async (response) => {
      ...
      await Promise.all([
        ...
        invalidateTicketQueries(queryClient, 'non-warranty'),
      ...
      toast.success(translate('notifications.repairTicketConverted'))
      setConvertModalOpen(false)
      if (response.data?.data?.work_order?.id) {
        navigate(`/work-orders/${response.data.data.work_order.id}`)
      }
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || translate('errors.failedToConvertRepairTicket'))
    },
  })
  ...
  const handleSave = () => {
    ...
    if (form.brought_by !== ticket.data.brought_by) updates.brought_by = ticket.data.brought_by
  ...
  const handleConvertToWorkOrder = () => {
    convertToWorkOrder.mutate(convertForm)
  }
  ...
  if (!ticket.data) {
    return (
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Alert severity="error" sx={{ fontSize: '1.1rem', py: 2 }}>
          {translate('errors.repairTicketNotFound')}
        </Alert>
      </Container>
    )
  }
    ...
    <Box sx={{ p: 3, maxWidth: 1400, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <IconButton onClick={() => navigate('/non-warranty')} sx={{ color: 'primary.main' }}>
            <ArrowBackIcon />
          </IconButton>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Avatar
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          {canConvertTicket() && (
            ...
            >
              {translate('actions.convertToWorkOrder')}
            </Button>
          )}
          {canDeleteTicket() && (
            <Button
        ...
        <Grid container spacing={3} alignItems="center">
          ...
          <Grid item xs={12} md={3}>
            <Box sx={{ textAlign: 'center' }}>
              ...
              <Chip
                label={data.warranty_expiry_date && new Date(data.warranty_expiry_date) > new Date() ? translate('status.underWarranty') : translate('status.noWarranty')}
                color={data.warranty_expiry_date && new Date(data.warranty_expiry_date) > new Date() ? 'success' : 'default'}
                size="large"
                sx={{ fontSize: '1rem', py: 1 }}
              />
            </Box>
      ...
      <Paper elevation={1} sx={{ borderRadius: 2 }}>
        ...
        >
          ...
          <Tab 
            ...
            iconPosition="start"
          />
          {data.status === 'converted' && data.converted_to_work_order_id && (
            <Tab 
        ...
        <Box sx={{ p: 3 }}>
                  ...
                  <ListItem sx={{ px: 0 }}>
                    ...
                    <ListItemText 
                      ...
                      secondary={
                        ...
                        `${translate('status.expires')}: ${formatDate(data.warranty_expiry_date)}` : 
                        translate('status.noWarranty')
                      }
                    />
                  </ListItem>
                </List>
          ...
          {activeTab === 3 && data.status === 'converted' && data.converted_to_work_order_id && (
            <Grid container spacing={3}>
              <Grid item xs={12} md={8}>
                <List>
                  <ListItem sx={{ px: 0 }}>
                    ...
                    <ListItemText 
                      primary={
                        <Button
                          component={Link}
                          to={`/work-orders/${data.converted_to_work_order_id}`}
                          size="large"
                          sx={{ textTransform: 'none' }}
                        >
                          {data.converted_work_order_formatted_number || `#${data.converted_to_work_order_id}`}
                        </Button>
                      }
      ...
      >
        <DialogTitle>
          {translate('dialogs.convertToWorkOrder')}
        </DialogTitle>
        <DialogContent>
            ...
            <Grid item xs={12}>
              <Alert severity="info">
                {(user?.role === 'admin' || user?.role === 'manager') 
                  ? translate('dialogs.convertToWorkOrderConfirmationManagerAdmin')
                  : translate('dialogs.convertToWorkOrderConfirmation')
                }
              </Alert>
            </Grid>
            <Grid item xs={12} md={6}>
        ...
        <DialogActions>
          ...
          <Button 
            ...
            variant="contained"
            disabled={convertToWorkOrder.isLoading}
            sx={{ bgcolor: 'success.main', '&:hover': { bgcolor: 'success.dark' } }}
          >
            {convertToWorkOrder.isLoading ? <CircularProgress size={24} /> : translate('actions.convert')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        <DialogTitle>{translate('dialogs.deleteRepairTicket')}</DialogTitle>
        <DialogContent>
          <Typography>
            {translate('dialogs.deleteRepairTicketConfirmation', { id: data.ticket_number || data.id })}
          </Typography>
        </DialogContent>
        <DialogActions>
