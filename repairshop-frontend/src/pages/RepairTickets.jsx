...
const RepairTickets = () => {
  ...
  const {
    ...
    queryKey: ['repair-tickets', queryParams.toString()],
    queryFn: async () => {
      const response = await api.get(`/repairTickets?${queryParams.toString()}`);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      await api.delete(`/repairTickets/${id}`);
    },
    onSuccess: async () => {
      // Invalidate and refetch ticket queries
      await invalidateTicketQueries(queryClient, 'non-warranty');
  ...
  const convertMutation = useMutation({
    mutationFn: async ({ ticketId, formData }) => {
      const response = await api.post(`/repairTickets/${ticketId}/convert`, formData);
      return response.data;
    },
    onSuccess: async () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm(translate('dialogs.deleteRepairTicketConfirmation'))) {
      try {
  ...
  const handleMenuAction = (action) => {
    ...
    switch (action) {
      case 'view':
        ...
        break;
      case 'edit':
        navigate(`/repair-tickets/${selectedTicketForMenu.id}/edit`);
        break;
      case 'convert':
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Box display="flex" gap={2} flexWrap="wrap" alignItems="center">
          <TextField
            size="small"
            placeholder={translate('navigation.searchRepairTicketsPlaceholder')}
            value={search}
      ...
      <Dialog 
        ...
        fullWidth
      >
        <DialogTitle>{translate('dialogs.convertToWorkOrder')}</DialogTitle>
        <DialogContent>
        ...
        {selectedTicketForMenu?.status === 'intake' && (
          <>
            ...
            <MenuItem onClick={() => handleMenuAction('convert')}>
              ...
              <ListItemText>{translate('actions.convertToWorkOrder')}</ListItemText>
            </MenuItem>
