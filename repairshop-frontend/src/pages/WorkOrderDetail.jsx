...
import { invalidateDashboardCache, invalidateWorkOrderQueries } from '../utils/cacheUtils.js'
...
export default function WorkOrderDetail() {
  ...
  const workOrderQuery = useQuery({
    ...
    queryFn: async () => (await api.get(`/workOrders/${id}`)).data.data,
  })

  const notesQuery = useQuery({
    queryKey: ['workOrderNotes', id],
    queryFn: async () => (await api.get(`/workOrderNotes/${id}`)).data,
  })

  const inventoryQuery = useQuery({
    queryKey: ['workOrderInventory', id],
    queryFn: async () => (await api.get(`/workOrderInventory/${id}`)).data,
  })
  ...
  const updateWorkOrder = useMutation({
    mutationFn: (updates) => api.patch(`/workOrders/${id}`, updates).then(r => r.data),
    onSuccess: async () => {
      // Invalidate the specific work order
      await queryClient.invalidateQueries({ queryKey: ['workOrder', id] });
      
      // Invalidate and refetch dashboard and work orders caches
      await Promise.all([
        invalidateDashboardCache(queryClient),
        invalidateWorkOrderQueries(queryClient, 'non-warranty')
      ]);
      
      toast.success(translate('notifications.workOrderUpdated'))
    },
  })

  const deleteWorkOrder = useMutation({
    mutationFn: () => api.delete(`/workOrders/${id}`).then(r => r.data),
    onSuccess: async (data) => { 
      toast.success(translate('notifications.workOrderDeleted')); 
      
      // Invalidate all repair tickets queries (including those with query parameters)
      await queryClient.invalidateQueries({ 
        predicate: (query) => query.queryKey[0] === 'repair-tickets' 
      });
      
      // Invalidate all work orders queries
      await queryClient.invalidateQueries({ 
        predicate: (query) => query.queryKey[0] === 'work-orders' 
      });
  ...
  const addNote = useMutation({
    mutationFn: (content) => api.post('/workOrderNotes', { work_order_id: id, content }).then(r => r.data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['workOrderNotes', id] })
  ...
  const addInventory = useMutation({
    mutationFn: (data) => api.post('/workOrderInventory', { work_order_id: id, ...data }).then(r => r.data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['workOrderInventory', id] })
  ...
  const calculateTotalCost = () => {
    const laborCost = (Number(form.labor_hours) || 0) * (Number(form.labor_rate) || 0)
    const partsCost = Number(form.quote_subtotal_parts) || 0
    const troubleshootingCost = Number(form.troubleshooting_fee) || 0
    return laborCost + partsCost + troubleshootingCost
  }
  ...
  const isTicket = workOrder?.status === 'intake'
  ...
  const onSave = () => {
    ...
    // Always calculate and include the total cost
    const calculatedTotal = calculateTotalCost()
    updates.quote_total = calculatedTotal
  ...
  const handleMoveToWorkOrder = () => {
    updateWorkOrder.mutate({ status: 'pending' })
  }
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1400, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            >
              {isTicket ? <TicketIcon /> : <AssignmentIcon />}
            </Avatar>
            <Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                <Typography variant="h4" component="h1" sx={{ fontWeight: 600 }}>
                  {isTicket ? translate('navigation.repairTickets') : translate('navigation.workOrders')} {workOrder.formatted_number || `#${workOrder.id}`}
                </Typography>
              </Box>
              <Typography variant="body2" color="text.secondary">
                {translate('tableHeaders.created')} {workOrder.created_at ? formatDateTime(workOrder.created_at) : translate('common.unknown')}
              </Typography>
            </Box>
          </Box>
        </Box>
          ...
          {!isEditing ? (
            ...
            <>
              ...
              <Button
                ...
                onClick={onSave}
              >
                {updateWorkOrder.isLoading ? translate('common.saving') : translate('actions.save')}
              </Button>
            </>
          )}
             ...
             <IconButton
               color="primary"
               onClick={() => window.open(`/api/print/${isTicket ? 'ticket' : 'workorder'}/${workOrder.id}`, '_blank')}
             >
               <PrintIcon />
             </IconButton>
           </Tooltip>
        ...
        <Box sx={{ flex: { xs: '1', md: '0 0 66.667%' }, maxWidth: { xs: '100%', md: '66.667%' } }}>
          <Card sx={{ height: '100%' }}>
            <CardContent>
                 ...
                 <DescriptionIcon />
                 {translate('common.workOrderDetails')}
               </Typography>
              ...
              <Box sx={{ mb: 4 }}>
               ...
               <Box sx={{ mb: 4 }}>
                 <Typography variant="subtitle1" sx={{ mb: 2, fontWeight: 600, color: 'primary.main' }}>
                   {translate('common.costInformation')}
                 </Typography>
                 <Grid container spacing={2}>
                   <Grid xs={12} sm={6}>
                     <TextField
                       ...
                       step="0.01"
                       min="0"
                       label={translate('forms.laborHours')}
                   ...
                   <Grid xs={12} sm={6}>
                     <TextField
                       ...
                       step="0.01"
                       min="0"
                       label={translate('forms.laborRate')}
                   ...
                   <Grid xs={12} sm={6}>
                     <TextField
                       ...
                       step="0.01"
                       min="0"
                       label={translate('forms.troubleshootingFee')}
                       value={form.troubleshooting_fee}
                       disabled={!isEditing}
                       onChange={(e) => setForm(f => ({ ...f, troubleshooting_fee: e.target.value }))}
                       helperText={translate('forms.troubleshootingFeeHelp')}
                       InputProps={{
                         startAdornment: <MoneyIcon sx={{ mr: 1, color: 'text.secondary' }} />,
                       }}
                     />
                   ...
                   <Grid xs={12} sm={6}>
                     <TextField
                       ...
                       step="0.01"
                       min="0"
                       label={translate('forms.partsSubtotal')}
                       value={form.quote_subtotal_parts}
                       disabled={true}
                       helperText={translate('forms.partsSubtotalHelp')}
                       InputProps={{
                 ...
                 {inventoryQuery.isLoading ? (
                   ...
                   <TableContainer component={Paper} variant="outlined" sx={{ maxHeight: 400 }}>
                     <Table size="small" stickyHeader>
                       <TableHead>
                         <TableRow>
                           <TableCell sx={{ fontWeight: 600, bgcolor: 'primary.main', color: 'white' }}>{translate('forms.itemName')}</TableCell>
                           <TableCell align="right" sx={{ fontWeight: 600, bgcolor: 'primary.main', color: 'white' }}>{translate('forms.quantity')}</TableCell>
                           <TableCell align="right" sx={{ fontWeight: 600, bgcolor: 'primary.main', color: 'white' }}>{translate('forms.unitPrice')}</TableCell>
                           <TableCell align="right" sx={{ fontWeight: 600, bgcolor: 'primary.main', color: 'white' }}>{translate('common.totalPrice')}</TableCell>
                           <TableCell align="center" sx={{ fontWeight: 600, bgcolor: 'primary.main', color: 'white' }}>{translate('tableHeaders.actions')}</TableCell>
                         </TableRow>
                       </TableHead>
                       <TableBody>
                         {inventoryQuery.data.map((item, index) => (
                           <TableRow key={item.id} hover sx={{ '&:nth-of-type(odd)': { bgcolor: 'grey.50' } }}>
                             ...
                             <TableCell align="center">
                               <Tooltip title={translate('actions.removePart')}>
                                 <IconButton
                                   size="small"
                                   color="error"
                                   onClick={() => {
               ...
               {/* Cost Summary - Moved to Bottom */}
               <Box sx={{ mb: 4 }}>
                 <Typography variant="subtitle1" sx={{ mb: 2, fontWeight: 600, color: 'primary.main' }}>
                   {translate('common.costSummary')}
                 </Typography>
                     ...
                     <Grid container spacing={3}>
                       ...
                       <Grid xs={12} sm={6} md={3}>
                         <Box sx={{ textAlign: 'center', p: 2, bgcolor: 'white', borderRadius: 1, border: '1px solid', borderColor: 'grey.300' }}>
                           ...
                           <Typography variant="caption" color="text.secondary">
                             {inventoryQuery.data?.length || 0} {translate('common.partsUsed')}
                           </Typography>
                         </Box>
                       </Grid>
                       ...
                       <Grid xs={12} sm={6} md={3}>
                         <Box sx={{ textAlign: 'center', p: 2, bgcolor: 'primary.main', borderRadius: 1, color: 'white' }}>
                           <ReceiptIcon sx={{ fontSize: 32, color: 'white', mb: 1 }} />
                           <Typography variant="body2" sx={{ mb: 1, opacity: 0.9 }}>
                             {translate('common.totalCost')}
                           </Typography>
                           <Typography variant="h5" sx={{ fontWeight: 700 }}>
                             ${calculateTotalCost().toFixed(2)}
                           </Typography>
                           <Typography variant="caption" sx={{ opacity: 0.8 }}>
                             {translate('common.finalAmount')}
                           </Typography>
                         </Box>
                       </Grid>
                     </Grid>
              ...
              <Box sx={{ mt: 3, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                {isTicket && !isEditing && canEdit && (
                  <Button
                    ...
                    disabled={updateWorkOrder.isLoading}
                  >
                                         {translate('common.moveToWorkOrders')}
                   </Button>
                )}
              </Box>
            </CardContent>
      ...
      {/* Delete Confirmation Dialog */}
         ...
         <DialogTitle>{isTicket ? translate('common.deleteTicket') : translate('common.deleteWorkOrder')}</DialogTitle>
         <DialogContent>
           <Typography>
             {isTicket ? translate('common.deleteTicketConfirmation') : translate('common.deleteWorkOrderConfirmation')}
           </Typography>
         </DialogContent>
         <DialogActions>
           <Button onClick={() => setDeleteDialogOpen(false)}>{translate('actions.cancel')}</Button>
          <Button
            color="error"
            variant="contained"
            onClick={() => {
          ...
          >
            {deleteWorkOrder.isLoading ? translate('common.deleting') : translate('actions.delete')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={noteDialogOpen} onClose={() => setNoteDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <AddIcon />
          {translate('dialogs.addTechnicianNote')}
        </DialogTitle>
        <DialogContent>
          <Grid xs={12}>
            <TextField
              ...
              multiline
              rows={4}
              label={translate('forms.noteContent')}
              value={newNote}
              onChange={(e) => setNewNote(e.target.value)}
              placeholder={translate('forms.enterNoteHere')}
              sx={{ mt: 1 }}
            />
          </Grid>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setNoteDialogOpen(false)}>{translate('actions.cancel')}</Button>
          <Button
            variant="contained"
            onClick={handleAddNote}
            disabled={addNote.isLoading || !newNote.trim()}
          >
            {addNote.isLoading ? translate('common.adding') : translate('actions.addNote')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={inventoryDialogOpen} onClose={() => setInventoryDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setInventoryDialogOpen(false)}>{translate('actions.cancel')}</Button>
          <Button
            variant="contained"
            onClick={handleAddInventory}
            disabled={addInventory.isLoading || !newInventory.inventory_id || !newInventory.quantity}
          >
            {addInventory.isLoading ? translate('common.adding') : translate('actions.addPart')}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
...
function MachineInfo({ machineId, workOrder }) {
  const { translate, formatDate, formatDateTime, formatDateTimeForInput, parseFormattedDateTime } = useLanguage()
    ...
    <List dense>
      <ListItem sx={{ px: 0 }}>
        ...
        <ListItemText 
          ...
          secondary={translate('forms.machineName')}
        />
      </ListItem>
      ...
      {machine.catalogue_number && (
        <ListItem sx={{ px: 0 }}>
          ...
          <ListItemText 
            ...
            secondary={translate('forms.catalogueNumber')}
          />
        </ListItem>
      )}
      ...
      {machine.description && (
        <ListItem sx={{ px: 0 }}>
          ...
          <ListItemText 
            ...
            secondary={translate('forms.description')}
          />
        </ListItem>
      )}
