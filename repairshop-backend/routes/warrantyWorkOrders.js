  ...
  try {
    ...
    const result = await db.query(`
      SELECT DISTINCT
        ...
        wwoi.quantity as used_quantity
...
router.get('/', authenticateToken, async (req, res, next) => {
  ...
  try {
    ...
    if (search) {
      whereConditions.push(`(wwo.description ILIKE $${paramIndex} OR c.name ILIKE $${paramIndex} OR mm.name ILIKE $${paramIndex})`);
...
router.get('/:id/notes', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      `SELECT id, content as note, created_at, updated_at
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO warranty_work_order_notes (warranty_work_order_id, content)
       VALUES ($1, $2)
       RETURNING id, content as note, created_at, updated_at`,
      [id, note.trim()]
    );
