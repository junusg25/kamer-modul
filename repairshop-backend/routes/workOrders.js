...
router.get('/', authenticateToken, async (req, res, next) => {
  ...
  try {
    ...
    if (search) {
      whereConditions.push(`(wo.description ILIKE $${paramIndex} OR c.name ILIKE $${paramIndex} OR m.name ILIKE $${paramIndex})`);
...
router.patch('/bulk-update', authenticateToken, authorizeRoles('manager','admin'), async (req, res, next) => {
  try {
...
router.patch('/bulk-assign', authenticateToken, authorizeRoles('manager','admin'), async (req, res, next) => {
  try {
...
router.patch('/:id', authenticateToken, checkWorkOrderOwnership, [
  ...
  body('status').optional().isIn(['intake','quoted','awaiting_approval','declined','pending','in_progress','completed','ready_for_pickup']),
