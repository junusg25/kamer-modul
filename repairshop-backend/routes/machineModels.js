...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (search) {
      query += ` AND (name ILIKE $${params.length + 1} OR catalogue_number ILIKE $${params.length + 1} OR manufacturer ILIKE $${params.length + 1})`;
      params.push(`%${search}%`);
    }
    
    query += ` ORDER BY name, catalogue_number LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
    ...
    if (search) {
      countQuery += ` AND (name ILIKE $${countParams.length + 1} OR catalogue_number ILIKE $${countParams.length + 1} OR manufacturer ILIKE $${countParams.length + 1})`;
