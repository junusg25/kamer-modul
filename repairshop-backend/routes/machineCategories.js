...
router.post('/', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      'INSERT INTO machine_categories (name, description) VALUES ($1, $2) RETURNING *',
      [name, description]
    )
...
router.put('/:id', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      'UPDATE machine_categories SET name = $1, description = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3 RETURNING *',
      [name, description, id]
    )
