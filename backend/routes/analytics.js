  ...
  try {
    ...
    const trendsQuery = `
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN am.is_sale = true THEN am.customer_id END) as unique_customers,
        COALESCE(SUM(CASE WHEN am.is_sale = true THEN am.sale_price END) / NULLIF(COUNT(DISTINCT CASE WHEN am.is_sale = true THEN am.customer_id END), 0), 0) as avg_revenue_per_customer,
    ...
    res.json({
      ...
      data: result.rows.map(row => ({
        ...
        avgRevenuePerCustomer: parseFloat(row.avg_revenue_per_customer),
        newMachinesSold: parseInt(row.new_machines_sold),
        usedMachinesSold: parseInt(row.used_machines_sold),
        topSellingModel: row.top_selling_model || 'N/A'
      }))
    });
  } catch (error) {
