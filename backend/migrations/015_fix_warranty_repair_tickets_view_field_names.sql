-- Migration: Fix warranty_repair_tickets_view field names to match frontend expectations
-- Date: 2025-01-14
-- Purpose: Change converted_warranty_work_order_formatted_number to converted_work_order_formatted_number
...
-- Recreate the warranty_repair_tickets_view with corrected field names
...
SELECT 
    ...
    wrt.machine_id,
    -- Machine details from sold_machines (sm) or machines (rm)
    ...
    -- FIXED: Use same field name as repair_tickets_view
    wwo.formatted_number AS converted_work_order_formatted_number,
    wwo.year_created AS converted_work_order_year_created,
...
-- Join for sold_machines (sold machines)
...
-- Join for machines (repair machines)
