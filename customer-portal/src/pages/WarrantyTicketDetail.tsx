...
import { ArrowLeft, Shield, Calendar, User, Package, FileText, AlertCircle, CheckCircle } from 'lucide-react';

interface WarrantyTicketDetail {
  id: number;
  ...
  converted_to_warranty_work_order_id?: number;
...
export default function WarrantyTicketDetail() {
  ...
  const fetchTicketDetail = async () => {
    ...
    try {
      const result = await apiService.getItemDetail('warranty_ticket', id!);
      
      if (result.data.type !== 'warranty_ticket') {
      ...
      setError(err.message || 'Failed to load warranty ticket details');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading warranty ticket details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="bg-card border border-border p-8 rounded-lg shadow-md text-center max-w-md">
          ...
          <p className="text-foreground mb-4">{error || 'Warranty ticket not found'}</p>
          <button
  ...
  return (
    <MainLayout>
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Warranty Repair Ticket</p>
              </div>
            </div>
        ...
        {/* Conversion Notice */}
        {ticket.converted_to_warranty_work_order_id && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 mb-6">
            <div className="flex items-center space-x-2">
              <CheckCircle className="w-5 h-5 text-green-600" />
              <div>
                <h3 className="font-semibold text-foreground">Converted to Warranty Work Order</h3>
                <p className="text-sm text-muted-foreground">
                  This ticket has been converted to warranty work order{' '}
                  <button
                    onClick={() => navigate(`/warranty-work-orders/${ticket.converted_to_warranty_work_order_id}`)}
            ...
            <div className="bg-card border border-border rounded-lg shadow p-6">
              ...
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p className="text-xs text-blue-800 dark:text-blue-200">
                  💡 This is a warranty repair - no charges will apply if approved
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <p className="text-foreground font-medium">Warranty Ticket Submitted</p>
            ...
            {ticket.converted_to_warranty_work_order_id && ticket.converted_at && (
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 rounded-full mt-2 bg-green-500"></div>
                <div className="flex-1">
                  <p className="text-foreground font-medium">Converted to Warranty Work Order</p>
                  ...
                  >
                    View Warranty Work Order {ticket.converted_work_order_formatted_number} →
                  </button>
                </div>
              </div>
            )}
