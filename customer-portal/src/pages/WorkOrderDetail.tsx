...
import { ArrowLeft, Wrench, Calendar, User, DollarSign, Clock, Package, FileText, AlertCircle } from 'lucide-react';
...
export default function WorkOrderDetail() {
  ...
  const fetchWorkOrderDetail = async () => {
    ...
    try {
      const result = await apiService.getItemDetail('work_order', id!);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="bg-card border border-border p-8 rounded-lg shadow-md text-center max-w-md">
          ...
          <p className="text-foreground mb-4">{error || 'Work order not found'}</p>
          <button
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-3">
              <Wrench className="w-8 h-8" style={{ color: '#ff8800' }} />
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-card border border-border rounded-lg shadow p-6">
            ...
            <div className="space-y-3">
              {workOrder.labor_hours && workOrder.labor_rate && (
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Labor ({workOrder.labor_hours}h × {formatCurrency(workOrder.labor_rate)}/h)</span>
                  <span className="text-sm font-medium text-foreground">
                    {formatCurrency(workOrder.labor_hours * workOrder.labor_rate)}
                  </span>
                </div>
              )}
              ...
              {workOrder.troubleshooting_fee && (
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Troubleshooting Fee</span>
                  <span className="text-sm font-medium text-foreground">
                    {formatCurrency(workOrder.troubleshooting_fee)}
                  </span>
                </div>
              )}
              ...
              <div className="pt-3 border-t border-border">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold" style={{ color: '#ff8800' }}>
                    {formatCurrency(workOrder.total_cost || 0)}
                  </span>
                </div>
              </div>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  ...
                  <tr className="bg-muted/30">
                    ...
                    <td className="py-3 px-4 text-right font-bold" style={{ color: '#ff8800' }}>
                      {formatCurrency(calculatePartsTotal())}
                    </td>
                  </tr>
                </tbody>
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <p className="text-foreground font-medium">Work Order Created</p>
